...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(label_filename, "rb") as f:
        f.read(8)  # Magic and num samples
...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.mean(np.log(np.exp(Z).sum(axis=1)) - Z[np.arange(y.size), y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):  # Assume batch evenly divides
        X_batch = X[i : i + batch]
        y_batch = y[i : i + batch]
        Z_batch = np.exp(X_batch @ theta)
        Z_batch /= Z_batch.sum(axis=1, keepdims=True)
        IY = np.zeros(Z_batch.shape)
        IY[np.arange(y_batch.size), y_batch] = 1
        theta -= lr / batch * X_batch.T @ (Z_batch - IY)
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):  # Assume batch evenly divides
        ...
        y_batch = y[i : i + batch]
        Z1 = X_batch @ W1
        Z1 = np.maximum(Z1, np.zeros(Z1.shape))
        G2 = np.exp(Z1 @ W2)
